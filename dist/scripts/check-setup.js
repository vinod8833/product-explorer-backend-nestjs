...
async function checkSetup() {
    ...
    if (checkCommand('npx tsc --version')) {
        const tsVersion = (0, child_process_1.execSync)('npx tsc --version', { encoding: 'utf8' }).trim();
    ...
    if (checkCommand('npx playwright --version')) {
        const playwrightVersion = (0, child_process_1.execSync)('npx playwright --version', { encoding: 'utf8' }).trim();
    ...
    try {
        ...
        if (fs.existsSync(playwrightCacheDir)) {
            ...
            const hasChromium = browsers.some((dir) => dir.startsWith('chromium'));
            const hasFirefox = browsers.some((dir) => dir.startsWith('firefox'));
