...
export async function seedLargeDataset(dataSource: DataSource, count: number = 1200) {
  ...
  for (let batch = 0; batch < batches; batch++) {
    ...
    for (let i = batchStart; i < batchEnd; i++) {
      ...
      const detail = {
        ...
        ratingsAvg: Math.round((Math.random() * 2 + 3) * 10) / 10, // 3.0 to 5.0
...
if (require.main === module) {
  AppDataSource.initialize().then(async () => {
    try {
      ...
      await seedLargeDataset(AppDataSource, count);
