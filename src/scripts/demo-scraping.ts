...
async function runDemo() {
  ...
  const enhancedService = app.get(WorldOfBooksEnhancedService);
  ...
  try {
    ...
    const collectionBooks = await apiService.getProductsByCollection(
      WorldOfBooksApiService.COLLECTIONS.SALE_COLLECTION_1, 
      0, 
      3
    );
    ...
    const categoryJob = await scrapingService.triggerCategoryScrape(
